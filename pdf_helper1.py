...
def transform_extracted_data(extracted_data):
    ...
    try:
        ...
        for key, value in extracted_data.items():
            ...
            # ✅ Fixed: Collect fields for P_Information (normalized keys)
            norm_key = key.strip().lower().replace(" ", "_")
            if norm_key in ('reason_for_referral', 'relevant_medical_conditions', 'related_information'):
                json_for_excel[key] = value
                p_information_parts.append(value)

        # ✅ Build P_Information exactly like reference code
        json_for_excel['P_Information'] = ', '.join(
            part.replace('Select from drop down ', '') for part in p_information_parts if part
        )

        if json_for_excel.get('P_Surname') == 'Wilding':
